...
namespace amd64::ir::parser
{
    ...
    {
        kw_fn, kw_ret, kw_jmp, kw_brif, kw_call, kw_native,
        kw_iconst, kw_iadd, kw_isub, kw_imul, kw_idiv, kw_icmp,
        kw_i64, kw_i32, kw_bool, kw_ptr, kw_bb,
        identifier, integer, percent, at, dot, comma,
    ...
    inline const std::unordered_map<std::string, token_type_t> keywords {
        { "fn", token_type_t::kw_fn }, { "ret", token_type_t::kw_ret },
        { "jmp", token_type_t::kw_jmp }, { "brif", token_type_t::kw_brif },
        { "call", token_type_t::kw_call }, { "native", token_type_t::kw_native },
        { "iconst", token_type_t::kw_iconst }, { "iadd", token_type_t::kw_iadd },
        { "isub", token_type_t::kw_isub }, { "imul", token_type_t::kw_imul },
        { "idiv", token_type_t::kw_idiv }, { "icmp", token_type_t::kw_icmp },
        { "i64", token_type_t::kw_i64 }, { "i32", token_type_t::kw_i32 },
        { "bool", token_type_t::kw_bool }, { "ptr", token_type_t::kw_ptr },
    };
    ...
    public:
        ...
        {
            skip_ws( );
            if ( m_pos >= m_source.size( ) ) return { token_type_t::eof, std::string( "" ), m_pos };

            const auto start = m_pos;
            const char c = m_source[ m_pos ];

            if ( c == '%' ) { ++m_pos; return process_int( token_type_t::percent, start ); }
            if ( c == '@' ) { ++m_pos; return { token_type_t::at, std::string( "@" ), start }; }
            if ( c == '.' ) { ++m_pos; return { token_type_t::dot, std::string( "." ), start }; }
            if ( c == ',' ) { ++m_pos; return { token_type_t::comma, std::string( "," ), start }; }
            if ( c == ':' ) { ++m_pos; return { token_type_t::colon, std::string( ":" ), start }; }
            if ( c == '(' ) { ++m_pos; return { token_type_t::lparen, std::string( "(" ), start }; }
            if ( c == ')' ) { ++m_pos; return { token_type_t::rparen, std::string( ")" ), start }; }
            if ( c == '{' ) { ++m_pos; return { token_type_t::lbrace, std::string( "{" ), start }; }
            if ( c == '}' ) { ++m_pos; return { token_type_t::rbrace, std::string( "}" ), start }; }
            if ( c == '=' ) { ++m_pos; return { token_type_t::equals, std::string( "=" ), start }; }
            ...
            if ( c == '-' || is_digit( c ) ) return process_int( token_type_t::integer, start );
            if ( is_ident( c ) ) return process_ident( start );
    ...
    private:
        ...
        token_t process_int( const token_type_t typ, const std::size_t start )
        {
            ...
            const bool is_hex = m_pos + 1 < m_source.size( ) &&
                                 m_source[ m_pos ] == '0' &&
            ...
            {
                while ( m_pos < m_source.size( ) && is_digit( m_source[ m_pos ] ) ) ++m_pos;
            }
            ...
            const auto [ptr, ec] = std::from_chars(
                m_source.data( ) + base_start, m_source.data( ) + m_pos, value, is_hex ? 16 : 10 );
        ...
        {
            while ( m_pos < m_source.size( ) && is_any( m_source[ m_pos ] ) ) ++m_pos;
            ...
            if ( value.starts_with( "bb" ) && value.size( ) > 2 &&
                ...
                return { token_type_t::kw_bb, static_cast<std::int64_t>( std::stoll( value.substr( 2 ) ) ), start };
    ...
    {
    public:
        explicit parser_t( std::string_view source )
            : m_lexer( source ), m_current( m_lexer.next( ) )
        { }
    ...
    private:
        ...
        void check_id( const std::int64_t parsed_id, const Value real )
        {
            if ( static_cast<Value>( parsed_id ) != real )
                throw parse_error( "id mismatch: text says %" + std::to_string( parsed_id ) +
                                    " but builder produced %" + std::to_string( real ) );
        ...
        type_t parse_type( )
        {
            switch ( m_current.type )
            {
                ...
                default:
                    throw parse_error( "expected a type at position " + std::to_string( m_current.pos ) );
            }
        }
        ...
        void parse_function( module_t& mod )
        {
            ...
            {
                if ( !param_types.empty( ) ) expect( token_type_t::comma, "','" );
            ...
            if ( param_ids.size( ) != fn.args.size( ) )
                throw parse_error( "function declares " + std::to_string( param_ids.size( ) ) +
                                    " parameter(s) but has " + std::to_string( fn.args.size( ) ) );
        ...
        void parse_block( function_t& fn, bool& first_block )
        {
            ...
            if ( static_cast<std::size_t>( declared_idx ) != block_idx )
                throw parse_error( "block index mismatch: text says bb" + std::to_string( declared_idx ) +
                                    " but this is block " + std::to_string( block_idx ) );

            if ( block_idx != 0 ) fn.create_block( param_types );

            if ( param_ids.size( ) != fn.param_indices[block_idx].size( ) )
                throw parse_error( "block " + std::to_string( block_idx ) + " declares " +
                                    std::to_string( param_ids.size( ) ) + " parameter(s) but has " +
                                    std::to_string( fn.param_indices[block_idx].size( ) ) );
        ...
        std::vector<Value> parse_arg_list( )
        {
            ...
            while ( m_current.type != token_type_t::rparen )
            {
                if ( !args.empty( ) ) expect( token_type_t::comma, "','" );
        ...
        void parse_instruction( function_t& fn )
        {
            ...
            if ( m_current.type == token_type_t::kw_brif )
            {
                ...
                fn.brif( cond, static_cast<std::size_t>( true_blk ), static_cast<std::size_t>( false_blk ),
            ...
            switch ( m_current.type )
            {
                ...
                default:
                    throw parse_error( "unexpected instruction at position " + std::to_string( m_current.pos ) );
            }
        }

        static set_cc_kind parse_set_cc_kind( const std::string& s )
        {
            if ( s == "eq" ) return set_cc_kind::eq;
            if ( s == "ne" ) return set_cc_kind::ne;
            if ( s == "lt" ) return set_cc_kind::lt;
            if ( s == "le" ) return set_cc_kind::le;
            if ( s == "gt" ) return set_cc_kind::gt;
            if ( s == "ge" ) return set_cc_kind::ge;
