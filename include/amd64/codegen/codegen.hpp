...
namespace amd64::codegen
{
    ...
    struct gen_module_t
    {
        ...
        template <typename F>
        [[nodiscard]] F get_function( const std::string& sym ) const
        {
    ...
    public:
        ...
        void compile_function( function_t & func, gen_module_t& mod, const std::string& symbol )
        {
            ...
            {
                if ( !is_callee_register( reg ) ) continue;
            ...
            if ( frames_needed ) m_asm.enter( stk_size );

            std::vector< std::size_t > labels { };
            for ( auto i { 0uz }; i < func.blocks.size(  ); ++i ) labels.push_back( m_asm.label(  ) );
                    ...
                    std::visit( [&]( auto& i )
                    {
                        ...
                        else if constexpr ( std::is_same_v<T, i_call> )
                        {
                            ...
                            else
                            {
                                ...
                                if ( !fn ) throw std::runtime_error( "call target function not found: " + name );
                                m_asm.call( fn );
                            }
                        ...
                        else if constexpr ( std::is_same_v<T, i_brif> )
                        {
                            ...
                            const Register condition = in_register( result, i.condition );
