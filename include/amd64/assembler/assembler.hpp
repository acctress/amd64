...
namespace amd64::assembler
{
    ...
    public:
        ...
        [[nodiscard]] const void *entry_at( const std::size_t pos ) const noexcept { return m_buffer.data( ) + pos; }
        ...
        auto bind( const std::size_t label )
        {
            ...
            {
                const auto offset = static_cast< std::int32_t >( static_cast< std::int64_t >( m_labels[ label ].target.value( ) )
                                                                 - ( static_cast< std::int64_t >( fix ) + 4 ) );
        ...
        auto patch_at( const std::size_t pos, const std::span< const std::byte > bytes ) const { m_buffer.patch_at( pos, bytes ); }

        auto nop( const std::uint32_t count )
        {
            for ( auto i { 0uz }; i < count; ++i )
                m_buffer.write_byte( 0x90_b );
        }

        auto pop( const Register reg )
        {
            if ( ext( reg ) )
                m_buffer.write_byte( std::byte { rex( false, Register::rax, reg ) } );
        ...
        auto mov_reg_imm64( const Register reg, const std::int64_t imm )
        {
            m_buffer.write_bytes(
                { static_cast< std::byte >( rex( true, Register::rax, reg ) ), static_cast< std::byte >( 0xB8 + enc( reg ) ) } );
        ...
        {
            static constexpr std::array< std::uint8_t const, 10 > opcodes { 0x94, 0x95, 0x9C, 0x9E, 0x9F, 0x9D, 0x92, 0x96, 0x97, 0x93 };
    ...
    private:
        ...
        void emit_jcc( const std::initializer_list< std::byte > opcode, const std::size_t label )
        {
            ...
            {
                const auto offset = static_cast< std::int32_t >( static_cast< std::int64_t >( m_labels[ label ].target.value( ) )
                                                                 - ( static_cast< std::int64_t >( fix_pos ) + 4 ) );
